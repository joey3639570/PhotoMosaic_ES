...
def build_mosaic(result_queue, all_tile_data_large, original_img_large, blocks):
	...
	image_path_list = seperate_images(n_blocks = blocks)
	np.savetxt("image_list.csv", image_path_list, delimiter =", ",  fmt ='% s')

def compose(original_img, tiles, blocks):
	...
	try:
		...
		Process(target=build_mosaic, args=(result_queue, all_tile_data_large, original_img_large, blocks)).start()
...
def seperate_images(img_path=OUT_FILE, directory="mosaic_images", n_blocks=(8,10)):
    ...
    for i in range(0, n_blocks[1]):
        for j in range(0, n_blocks[0]):
            path = [os.path.join(f"{directory}",f"{j+1:02d}_{i+1:02d}.jpg")]
            print(path)
            image_path.append(os.path.join(cwd_path, path[0]))
            cv2.imwrite(path[0] , splitted_img[i][j])
    return image_path

def mosaic(img_path, tiles_path, n_blocks):
	image_data = TargetImage(img_path).get_data()
	tiles_data = TileProcessor(tiles_path).get_tiles()
	compose(image_data, tiles_data, n_blocks)

if __name__ == '__main__':
	if len(sys.argv) < 5:
		print('Usage: {} <image> <tiles directory> <blocks_x> <blocks_y>\r'.format(sys.argv[0]))
	else:
		blocks = (int(sys.argv[3]), int(sys.argv[4]))
		mosaic(sys.argv[1], sys.argv[2], blocks)
